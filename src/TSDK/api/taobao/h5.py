...
''' 
    ...
    def h5_token(self):
        ...
        logger.error('从cookie获取的token为空')
        return ''
    ...
    def sign(self, token: str, t: str, appkey: str, data: str, binary: bool = False):
        ...
        logger.debug('sign签名字符串:{signStr}',signStr=sign_str)
    ...
    def _execute(self, request_options: Any):
        ...
        logger.debug('{method}请求:{payload}', payload=payload, method=method.upper())
        # self.headers.update({
        ...
        try:
            ...
            if res.status_code != 200:
                logger.error('API请求失败:{api}', res.text)
            else:
                ret = resj.get('ret')
                if not any(filter(lambda retStatus: 'SUCCESS' in retStatus, ret)):
                    logger.error('API请求失败:{ret}', ret=ret)
                    if len(ret) >= 2:
    ...
    def _login_bofore(self):
        ...
        if res.status_code != 200:
            logger.error('初始化cooie失败，请检查原因:',res.text)
            return False
        # 提取csrf
        regex = re.findall(r'viewData = (\{.*\})',res.text)
        if len(regex) <= 0:
            logger.error('提取csrf失败')
    ...
    def qrLogin(self, timeout: int = 60, autoTrust: bool = False) -> bool:
        ...
        #     logger.error('获取登录二维码失败:{rtxt}', rtxt=res.text)
        #     return False
        ...
        while timeout > 0:
            ...
            if qrCodeStatus == taobao.QrStatus.已确认.value:
                ...
                #     logger.debug('安全登录跳转返回:{tt}', tt=safeRes.text)

                #     data, res = self.getUserSimple()
                #     if not data.get('data'):
                #         logger.error('需要安全验证登录:{rtext}', rtext=mainRes.text)
                #         break
                if autoTrust:
                    # 自动信任
                ...
                logger.debug('二维码过期: {res}', res=checkData)
                break
            else:
                pass
    ...
    def qrCheck(self, lgToken: str, umidToken: str):
        ...
        logger.debug('检查qrcode登录状态:{resj}', resj=resj)
        return resj
    ...
    def qrNewCheck(self, t: int, ck: str, ):
        ...
        if hasError:
            logger.error('请求错误:{resj}', resj=resj)
        logger.debug(f'检查返回:{resj}')
    ...
    def __hookX5(self, res: Response, jumpUrl: str = ''):
        ...
        if not jumpUrl and 'x5referer' in res.text:
            ...
            if len(locationHref) <= 0:
                logger.error('提取跳转链接失败:{rtxt}',rtxt=res.text)
                return
            
            jumpUrl = locationHref[0] + quote(res.url)

        logger.debug('打开链接:{jumpUrl}', jumpUrl=jumpUrl)
        playwright = sync_playwright().start()
        iPhone13 = playwright.devices['iPhone 13']
        logger.debug('设备列表:{devices}',devices=playwright.devices)
    ...
    def __drag_element(self, page: Page, elementSelector: str = '#nc_1_n1z'):
        ...
        if not element:
            logger.error('未找到元素', elementSelector)
            return
    ...
    def handleResponse(self, response: PlayResponse):
        ...
        logger.debug('请求:{url}',url=response.url)
        logger.debug('提交数据:{data}',data=response.request.post_data)
    ...
    def jsvApiAutoCreate(self, urls:List[str], headless=False):
        ...
        logger.debug('打开链接:{jumpUrl}', jumpUrl=urls)
        playwright = sync_playwright().start()
        iPhone13 = playwright.devices['iPhone 13']
        logger.debug('设备列表:{devices}',devices=playwright.devices)
    ...
    def urlCreateFunc(self, api: str, method: str = 'get', func_name: str | None = None, desc: str = ''):
        ...
        if hasattr(self, func_name):
            logger.debug('已存在函数:{func_name}',func_name=func_name)
            return
